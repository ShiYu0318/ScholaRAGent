...
GET  /api/daily                  -> 個人化排序後的今日論文
GET  /api/trends                 -> 熱詞 + LSTM 預測（已有）
GET  /api/reading                -> 閱讀看板狀態
POST /api/reading    {id,state}  -> 更新看板
GET  /api/compare?ids=...        -> 方法×資料集×指標比較表
```
...
3. 維持原型定調，優先回 A3（BGE reranker）等後端功能，前端等 GraphRAG 就緒。
